...
const props = defineProps<{ item: Item, disabled?: boolean }>()
...
<template>
	...
	<div class="flex-1">
		...
		<p class="text-xs text-gray-500 dark:text-gray-300">Цена: {{ currencyFormatter(itemPrice) }}/{{ props.item.unit }}</p>
		...
		<div class="flex justify-between gap-2 md:gap-4 items-center mt-1">
			...
			<div>
			    <Button variant="outline" size="icon" class="size-8 md:size-10" @click="removeItem(props.item.id || 0)" :disabled="props.disabled">
