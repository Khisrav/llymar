...
const buttonClasses = computed(() => {
	const baseClasses = "group font-medium transition-all duration-300 focus:outline-none focus-visible:outline-2 focus-visible:outline-light-gold focus-visible:outline-offset-2 disabled:opacity-50 disabled:cursor-not-allowed";
	...
	const displayClasses = props.display === "block" ? "w-full flex" : "inline-flex"; 
...
<template>
	<component :is="href ? 'a' : 'button'" :href="href" :target="external ? '_blank' : undefined" :rel="external ? 'noopener noreferrer' : undefined" :class="buttonClasses" :disabled="disabled || loading" @click="handleClick">
		<!-- Left icon -->
		<component v-if="icon && iconPosition === 'left'" :is="icon" :class="['w-5 h-5', iconClasses, iconPosition === 'left' ? 'mr-3' : 'ml-3']" />
		...
		<!-- Right icon or arrow -->
		<component v-if="icon && iconPosition === 'right'" :is="icon" :class="['w-5 h-5', iconClasses, iconPosition === 'right' ? ($slots.default ? 'ml-3' : '') : $slots.default ? 'mr-3' : '']" />

		<!-- Default arrow for primary buttons -->
		<svg v-else-if="(variant === 'primary' && !icon) || showArrow" width="49" height="8" viewBox="0 0 49 8" fill="none" xmlns="http://www.w3.org/2000/svg" class="ml-3 md:ml-4" :class="iconClasses">
			<path
				d="M48.3536 4.35356C48.5488 4.1583 48.5488 3.84171 48.3536 3.64645L45.1716 0.464471C44.9763 0.269209 44.6597 0.269209 44.4645 0.464471C44.2692 0.659733 44.2692 0.976316 44.4645 1.17158L47.2929 4.00001L44.4645 6.82843C44.2692 7.02369 44.2692 7.34028 44.4645 7.53554C44.6597 7.7308 44.9763 7.7308 45.1716 7.53554L48.3536 4.35356ZM0 4L-5.73712e-08 4.5L48 4.50001L48 4.00001L48 3.50001L5.73712e-08 3.5L0 4Z"
